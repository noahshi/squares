...
import { nil, cons, len, equal, at, concat, rev, prefix, suffix,
...
describe('list', function() {
  ...
  it('prefix', function() {
    ...
    // 0-1-many: base case
    assert.deepEqual(prefix(0n, nil), nil);
    assert.deepEqual(prefix(0n, cons(3n, nil)), nil);
    // 0-1-many: one recursive call
    assert.deepEqual(prefix(1n, cons(3n, nil)), cons(3n, nil));
    assert.deepEqual(prefix(1n, l5), cons(1n, nil));
    // 0-1-many: many recursive calls
    assert.deepEqual(prefix(2n, l5), cons(1n, cons(2n, nil)));
    assert.deepEqual(prefix(4n, l5), cons(1n, cons(2n, cons(3n, cons(4n, nil)))));
    assert.deepEqual(prefix(5n, l5), l5);
  ...
  it('suffix', function() {
    ...
    // 0-1-many: base case
    assert.deepEqual(suffix(0n, nil), nil);
    assert.deepEqual(suffix(0n, cons(3n, nil)), cons(3n, nil));
    // 0-1-many: one recursive call
    assert.deepEqual(suffix(1n, cons(3n, nil)), nil);
    assert.deepEqual(suffix(1n, l5), cons(2n, cons(3n, cons(4n, cons(5n, nil)))));
    // 0-1-many: many recursive calls
    assert.deepEqual(suffix(2n, l5), cons(3n, cons(4n, cons(5n, nil))));
    assert.deepEqual(suffix(3n, l5), cons(4n, cons(5n, nil)));
    assert.deepEqual(suffix(5n, l5), nil);
